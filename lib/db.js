...
export async function ensureSchema() {
  if (!_initPromise) {
    ...
    _initPromise = (async () => {
      ...
      `;
      // Also migrate older comprovantes tables that may already exist.
      await s`ALTER TABLE comprovantes ADD COLUMN IF NOT EXISTS transaction_id TEXT`;
      await s`ALTER TABLE comprovantes ADD COLUMN IF NOT EXISTS cpf TEXT`;
      await s`ALTER TABLE comprovantes ADD COLUMN IF NOT EXISTS nome TEXT`;
      await s`ALTER TABLE comprovantes ADD COLUMN IF NOT EXISTS acordo TEXT`;
      await s`ALTER TABLE comprovantes ADD COLUMN IF NOT EXISTS file_name TEXT`;
      await s`ALTER TABLE comprovantes ADD COLUMN IF NOT EXISTS mime_type TEXT`;
      await s`ALTER TABLE comprovantes ADD COLUMN IF NOT EXISTS data BYTEA`;
      await s`ALTER TABLE comprovantes ADD COLUMN IF NOT EXISTS size_bytes INTEGER`;
      await s`ALTER TABLE comprovantes ADD COLUMN IF NOT EXISTS uploaded_at TIMESTAMPTZ DEFAULT NOW()`;
