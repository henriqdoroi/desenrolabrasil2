...
function secret() {
  // ADMIN_SESSION_SECRET is preferred when configured.
  // For this project, ADMIN_PASSWORD is a safe fallback so the
  // session system works without requiring another Vercel variable.
  const s = process.env.ADMIN_SESSION_SECRET || process.env.ADMIN_PASSWORD;
  if (!s) throw new Error('ADMIN_PASSWORD nao configurado');

  return crypto
    .createHash('sha256')
    .update('desenrolabrasil-admin-session:' + s)
    .digest('hex');
}
